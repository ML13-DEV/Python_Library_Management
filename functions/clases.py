#DEFINIMOS CLASES PARA BIBLIOTECA, USUARIO, AUTOR, LIBRO Y RESERVA
...
class Usuario():
    def __init__(self, id, nombre, apellido, direccion, correo):
        self.id = id
        self.nombre = nombre
    ...
    def __str__(self):
        return f"Usuario: {self.id} | Nombre y Apellido: {self.nombre} {self.apellido} | Dirección: {self.direccion} | Correo: {self.correo}"
...
class Reserva():
    def __init__(self,id_reserva, id_libro, dni, fecha_salida, devolucion):
        self.id = id_reserva
        self.libro = id_libro
        self.dni = dni
        self.salida = fecha_salida
        self.devolucion = devolucion
    
    def __str__(self):
        return f"Libro {self.libro} | Usuario {self.dni} | Fecha de reserva {self.salida} | Fecha de devolución {self.devolucion}"
